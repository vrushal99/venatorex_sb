  ...
  function beforeLoad (context) {
    try {
      ...
      for(const iterator in finalObj){
        ...
        let coaLength = iterator.COA;
        log.debug('coaLength',coaLength)
        
        log.debug('iterator',iterator)
        var vendor = iterator.vendor
        var project_type = iterator.project_type
        var category = iterator.category
        var date_created = iterator.date_created
        var start_date = iterator.start_date
        var end_date = iterator.end_date
        var notes = iterator.notes
        var amount = iterator.amount
        ...
        for (let i = 0; i < searchResult.length; i++) {
          ...
          if (!_logValidation(objInMasterObj)) {
            ...
            finalVendorArr.push({
              ...
              subtotalDiff:objInMasterObj.getTotalCoaAmount-objInMasterObj.getTotalBillAmount
            });
